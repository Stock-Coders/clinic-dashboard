...
@section('title')
Edit {{ $payment->appointment->patient->first_name . ' ' . $payment->appointment->patient->last_name }}'s Payment
@endsection
@section('content')
        ...
        <div class="card shadow mb-4">
          <div class="card-header">
            <strong class="card-title fs-2">
                Edit Payment (ID: <span class="text-primary">{{ $payment->id }}</span>) for "<a class="text-decoration-underline" href="{{ route('patients.show', $payment->appointment->patient_id) }}">{{ $payment->appointment->patient->first_name . ' ' . $payment->appointment->patient->last_name }}</a>"
            </strong>
          </div>
          <div class="card-body">
            <div class="row">
