...
@section('title'){{ $patient->first_name . ' ' . $patient->last_name }}'s Appointments ({{ $patient->appointment()->count() }}) @endsection
@section('content')
      ...
      <div class="col-sm-12">
        <div class="card">
          <div class="card-header">
            <h5><a href="{{ route('patients.show', $patient->id) }}" class="text-decoration-underline fw-bold">{{ $patient->first_name . ' ' . $patient->last_name }}</a>'s Appointments ({{ $patient->appointment()->count() }})</h5>
            {{-- <span>
                <a href="{{ route('dashboard') }}">Dashboard</a> / <a href="{{ route('appointments.index') }}">Appointments</a> / All Appointments
            </span> --}}
            <div class="row">
                <div class="col-md-8">
                    <a href="{{ route('dashboard') }}">Dashboard</a> / All Appointments
                </div>
                <div class="col-md-1">
                    <a href="{{ route('appointments.show-index.pdf', $patient->id) }}" target="_blank" class="btn btn-secondary-gradien">PDF/Print</a>
                </div>
                <div class="col-md-3">
          ...
          <div class="card-body">
            <div class="table-responsive">
              <table class="display" id="basic-1">
                ...
                <tbody>
                @foreach ($appointmentsOfPatient as $appointment)
                <tr>
