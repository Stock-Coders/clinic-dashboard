...
@section('title') All Receipts ({{ \App\Models\Payment::count() }}) @endsection
@section('content')
              ...
              <table class="table table-bordered">
                ...
                <tr>
                    ...
                    <th class="hidden-print-view">
                        <div class="d-flex justify-content-between">
                            <a class="btn btn-warning text-dark btn-md m-1 px-3" href="{{ route('payments.show', [$payment->id, $payment->patient->first_name]) }}"title="{{ $payment->patient->first_name .' '. $payment->patient->last_name }}">
                                <i class="icofont icofont-open-eye f-24"></i>
                            </a>
                            <a class="btn btn-primary btn-md m-1 px-3" href="{{ route('payments.edit', $payment->id) }}"title="Edit ({{ $payment->patient->first_name .' '. $payment->patient->last_name }})">
                                <i class="fa fa-edit f-18"></i>
                            </a>
                            <form action="{{ route('payments.destroy', $payment->id) }}" method="post">
                                @csrf
                                @method('DELETE')
                                <button type="submit" onclick="return confirm('Are you sure that you want to delete ({{ $payment->patient->first_name . ' '. $payment->patient->last_name }})?');"title="{{"Delete ($payment->patient->first_name $payment->patient->last_name)"}}" class="btn btn-danger btn-md m-1 px-3"><i class="fa fa-trash-o f-18"></i></button>
                            </form>
                        </div>
                    </th>
                </tr>
