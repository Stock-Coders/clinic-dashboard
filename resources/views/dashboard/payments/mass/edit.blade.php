...
@section('title')
Edit {{ $payment->patient->first_name . ' ' . $payment->patient->last_name }}'s Payment
@endsection
@section('content')
        ...
        <div class="card shadow mb-4">
          <div class="card-header">
            <strong class="card-title fs-2">
                Edit Payment (ID: <span class="text-primary fw-bold">{{ $payment->id }}</span>) for <a class="text-decoration-underline text-primary fw-bold" href="{{ route('patients.show', $payment->patient_id) }}">{{ $payment->patient->first_name . ' ' . $payment->patient->last_name }}</a>
            </strong>
          </div>
          <div class="card-body">
            <div class="row">
              <div class="col-md-12">
                <form action="{{ route('payments.update', $payment->id) }}" class="forms-sample" method="POST" id="alert-form">
                    @csrf
                    @method('PATCH')
                    @include('dashboard.payments.mass.form')
